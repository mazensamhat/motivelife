...
const FILLER_RE = /\b(um+|uh+|er+|ah+|hm+|like|you know|sort of|kind of|basically|actually|literally|i mean)\b/gi;
const HEDGE_RE = /\b(maybe|perhaps|i think|i guess|somewhat|probably|kind of|sort of|might|could be)\b/gi;
const STRONG_RE = /\b(i led|i built|i delivered|i achieved|i increased|i reduced|my result|i'm confident|we grew|i saved)\b/gi;
const STRUCTURE_RE = /\b(first|second|third|finally|because|therefore|as a result|for example)\b/gi;
const WARM_RE = /\b(thank|appreciate|grateful|mean(s)? a lot|care about|value|understand|hear you|I feel|I noticed)\b/gi;
const BLAME_RE = /\b(you always|you never|you should|your fault|why don't you)\b/gi;
function isRelationshipMode(mode) {
    return mode === "date_intro" || mode === "difficult_talk" || mode === "reconnection" || mode === "appreciation";
}
function isLeadershipMode(mode) {
    return (mode === "parenting_moment" ||
        mode === "team_feedback" ||
        mode === "hard_decision" ||
        mode === "motivate_team");
}
...
export function scoreVoicePractice(transcript, mode, durationSeconds) {
    ...
    const avgSentenceLen = sentences.length > 0 ? sentences.reduce((sum, s) => sum + s.split(/\s+/).length, 0) / sentences.length : wordCount;
    let confidence = 68;
    ...
    if (wordCount >= 80)
        ...
        confidence -= 12;
    if (mode === "negotiation" && /\b(value|impact|delivered|results|metrics|\d+%)\b/i.test(text))
        confidence += 6;
    if (isRelationshipMode(mode)) {
        confidence += countMatches(text, WARM_RE) * 3;
        confidence -= countMatches(text, BLAME_RE) * 8;
        if (mode === "date_intro" && /\?\s*$/.test(text))
            confidence += 5;
    ...
    if (isLeadershipMode(mode)) {
        ...
        if (mode === "parenting_moment" && /\b(i see|i understand|let's|together|calm)\b/i.test(text))
            confidence += 8;
        if (mode === "team_feedback" && /\b(specific|next time|i noticed|here's what)\b/i.test(text))
            confidence += 7;
        if (mode === "hard_decision" && /\b(because|decision|moving forward|priorit)\b/i.test(text))
            confidence += 5;
        if (mode === "motivate_team" && /\b(we|us|together|next|learned)\b/i.test(text))
            confidence += 6;
    }
    let clarity = 70;
    if (avgSentenceLen >= 10 && avgSentenceLen <= 22)
        clarity += 10;
    else if (avgSentenceLen > 32)
        clarity -= 12;
    else if (avgSentenceLen < 6 && wordCount > 20)
        clarity -= 8;
    ...
    if (isRelationshipMode(mode)) {
        if (overall >= 82)
            coachNote += "warm, clear, and ready to say out loud.";
        else if (overall >= 70)
            coachNote += "good heart — tighten wording and cut fillers for the real conversation.";
        else
            coachNote += "start with one honest feeling, one specific detail, then pause.";
    }
    else if (isLeadershipMode(mode)) {
        if (overall >= 82)
            coachNote += "steady leadership voice — clear, human, and actionable.";
        else if (overall >= 70)
            coachNote += "solid tone — slow down and land one concrete next step.";
        else
            coachNote += "lead with empathy, name the reality, then one clear path forward.";
    }
    else if (overall >= 82)
        coachNote += "polished delivery. You're interview-ready on this prompt.";
    else if (overall >= 70)
        coachNote += "solid foundation. One more rep focusing on fillers will level you up.";
    else
        coachNote += "good start. Slow down, structure your answer, and lead with your strongest result.";
    return {
        scores: {
            overall,
            confidence,
