import { parseOpenAiUsage } from "./openai-usage";
import { buildPersonaSystemPrompt, buildPersonaUserPayload, } from "./persona-prompt";
export { buildPersonaSystemPrompt, buildPersonaUserPayload, computeLifeEngineStreakUpdate, getLifeEngineStreakStatus, getGenerationCohort, } from "./persona-prompt";
/** Rule-based briefing when no LLM key is configured */
export function generateBriefing(context) {
    ...
    for (const event of upcomingToday.slice(0, missionTask ? 1 : 2)) {
        const label = event.hoursUntil < 1
            ? `Soon: ${event.title}`
            : `Today: ${event.title}`;
        if (!priorities.includes(label))
    ...
    while (priorities.length < 3 && activeGoals.length > priorities.length) {
        ...
        if (goal)
            priorities.push(`Make progress on: ${goal.title}`);
    }
...
function pushSuggestion(suggestions, item, max) {
    if (suggestions.length >= max)
        return;
    suggestions.push(item);
}
/** Collect suggestions, optionally filtered to one agent */
export function collectSuggestions(context, options) {
    const max = options?.limit ?? 3;
    const suggestions = [];
    const { goals, tasks, staleGoals, applications = [], moneyItems = [], calendarEvents = [], habits = [], healthItems = [], learningItems = [], emails = [], memories = [], } = context;
    const now = Date.now();
    const agent = options?.agent;
    const allow = (a) => !agent || agent === a;
    if (allow("CALENDAR")) {
    ...
    if (allow("MONEY")) {
        ...
        for (const item of moneyItems.filter((m) => m.type === "DEBT" && m.currentAmount > 0).slice(0, 1)) {
            if (suggestions.some((s) => s.agent === "MONEY" && agent === "MONEY"))
                break;
            pushSuggestion(suggestions, {
                agent: "MONEY",
    ...
    if (allow("CAREER")) {
        ...
        for (const app of applications.filter((a) => a.nextStep).slice(0, 1)) {
            if (suggestions.some((s) => s.title.includes(app.company)))
                continue;
            pushSuggestion(suggestions, {
                agent: "CAREER",
                title: app.nextStep,
    ...
    if (allow("GENERAL")) {
        for (const habit of habits.filter((h) => !h.doneToday && /mom|dad|call|family|friend|sarah|brother|sister/i.test(h.title)).slice(0, 1)) {
            pushSuggestion(suggestions, {
                ...
                reason: habit.daysSinceLastDone != null && habit.daysSinceLastDone >= 7
        ...
        for (const mem of memories.filter((m) => /call|mom|dad|family|friend|birthday|check.?in/i.test(m.content + m.title)).slice(0, 1)) {
            if (suggestions.some((s) => s.actionHref === "/memory"))
                break;
            pushSuggestion(suggestions, {
                agent: "GENERAL",
                title: mem.title,
    ...
    if (allow("GENERAL")) {
        for (const habit of habits.filter((h) => !h.doneToday && !/mom|dad|call|family/i.test(h.title)).slice(0, 1)) {
            pushSuggestion(suggestions, {
                ...
                reason: habit.daysSinceLastDone != null && habit.daysSinceLastDone >= 2
    ...
    if (allow("HEALTH")) {
        for (const item of healthItems.filter((h) => h.percentComplete != null && h.percentComplete < 80).slice(0, 1)) {
            pushSuggestion(suggestions, {
                ...
                reason: item.percentComplete != null && item.percentComplete > 0
    ...
    if (allow("LEARNING")) {
        for (const item of learningItems.filter((l) => (l.daysUntilTarget != null && l.daysUntilTarget >= 0 && l.daysUntilTarget <= 14 && l.progress < 80) ||
            (l.progress === 0 && l.daysSinceUpdate >= 7)).slice(0, 1)) {
            pushSuggestion(suggestions, {
                agent: "LEARNING",
                title: `Make progress on "${item.title}"`,
                reason: item.daysUntilTarget != null && item.daysUntilTarget >= 0 && item.daysUntilTarget <= 14
...
export function getBestSuggestionForAgent(context, agent) {
...
export function getDomainNextActionFromContext(context, agent, domainLabel) {
...
export function habitDoneToday(lastDoneAt, now = new Date()) {
    if (!lastDoneAt)
        return false;
    return startOfDayLocal(lastDoneAt).getTime() === startOfDayLocal(now).getTime();
}
export function habitDaysSinceLastDone(lastDoneAt, now = new Date()) {
    if (!lastDoneAt)
        return null;
    const diff = startOfDayLocal(now).getTime() - startOfDayLocal(lastDoneAt).getTime();
    return Math.floor(diff / (1000 * 60 * 60 * 24));
}
export function computeHabitCheckIn(lastDoneAt, frequency, currentStreak, bestStreak, now = new Date()) {
...
export function buildHealthItemContext(item) {
    const percentComplete = item.targetValue && item.targetValue > 0
...
export function buildLearningItemContext(item) {
    ...
    if (item.targetDate) {
        daysUntilTarget = Math.ceil((item.targetDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
    }
...
export function buildMoneyItemContext(item) {
    ...
    if (item.type === "SAVINGS" && item.targetAmount && item.targetAmount > 0) {
        ...
        if (item.targetDate) {
            const months = Math.max(1, (item.targetDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24 * 30));
...
export async function generateBriefingWithAI(context, apiKey) {
...
export function generateEveningReview(context) {
    ...
    if (pendingTasks.length > 0) {
        const next = pendingTasks.find((t) => t.isMission) ??
    ...
    const parts = [];
    if (userName)
        parts.push(`${userName}, here's your day in review.`);
    else
...
export async function generateEveningReviewWithAI(context, apiKey) {
...
export function generateWeeklyReview(context) {
    const { userName, tasksCompleted, activeGoals, momentsThisWeek, pendingTasks, goalsCompletedThisWeek = 0, avgGoalProgress = 0, preferences, beliefs = [], lifeEngineStreak = 0, lifeXpGainedThisWeek = 0, coachingDaysCompleted = 0, topXpDimensionLabel = null, voiceRecap = null, } = context;
    const firstName = userName?.split(" ")[0] ?? null;
    const prefs = preferences;
    const beliefHint = beliefs.length > 0 ? ` Keeping ${beliefs[0].label.toLowerCase()} in mind.` : "";
    const wins = [];
    if (tasksCompleted > 0) {
        wins.push(`Completed ${tasksCompleted} task${tasksCompleted === 1 ? "" : "s"} this week.`);
    }
    if (goalsCompletedThisWeek > 0) {
        wins.push(`Finished ${goalsCompletedThisWeek} goal${goalsCompletedThisWeek === 1 ? "" : "s"}.`);
    }
    ...
    if (lifeEngineStreak >= 3) {
        letterParagraphs.push(prefs?.reminderStyle === "statistics"
    ...
    if (coachingDaysCompleted > 0) {
        letterParagraphs.push(coachingDaysCompleted >= 5
    ...
    if (voiceRecap && voiceRecap.captureCount > 0) {
        const practicePart = voiceRecap.practiceCount > 0 && voiceRecap.avgPracticeScore
            ? ` ${voiceRecap.practiceCount} voice practice rep${voiceRecap.practiceCount === 1 ? "" : "s"} (avg ${Math.round(voiceRecap.avgPracticeScore)}/100).`
            : "";
        const moodPart = voiceRecap.topMoods.length > 0 ? ` Mood trend: ${voiceRecap.topMoods.slice(0, 2).join(", ")}.` : "";
        letterParagraphs.push(`You spoke to MotiveLife ${voiceRecap.captureCount} time${voiceRecap.captureCount === 1 ? "" : "s"} this week.${practicePart}${moodPart}`);
    ...
    if (prefs?.reminderStyle === "direct") {
        letterParagraphs.push(tasksCompleted > 0
    ...
    if (avgGoalProgress > 0) {
        letterParagraphs.push(`You completed about ${Math.round(avgGoalProgress)}% of your active goals on average.`);
    }
    else if (tasksCompleted > 0) {
        letterParagraphs.push(`You completed ${tasksCompleted} task${tasksCompleted === 1 ? "" : "s"} — momentum is building.`);
    }
    if (momentsThisWeek.length > 0) {
        letterParagraphs.push(`${momentsThisWeek.length} moment${momentsThisWeek.length === 1 ? "" : "s"} moved your life forward.`);
    }
    if (wins.length > 1) {
...
function injectVoiceRecapIntoLetter(paragraphs, fallbackParagraphs, voiceRecap) {
    if (!voiceRecap || voiceRecap.captureCount === 0)
        return paragraphs;
    const voiceParas = fallbackParagraphs.filter((p) => p.includes("spoke to MotiveLife") || p.startsWith("Voice highlight:"));
    if (voiceParas.length === 0)
        return paragraphs;
    const howeverIdx = paragraphs.findIndex((p) => p === "However…");
    if (howeverIdx === -1)
        return [...paragraphs, ...voiceParas];
    return [...paragraphs.slice(0, howeverIdx), ...voiceParas, ...paragraphs.slice(howeverIdx)];
}
export async function generateWeeklyReviewWithAI(context, apiKey) {
  ...
  "wins": string[1-3] (specific wins from the data)
        ...
        const parsed = JSON.parse(content);
        const usage = parseOpenAiUsage(data);
        const letterParagraphsRaw = parsed.letterParagraphs?.length && parsed.letterParagraphs.length >= 4
            ? parsed.letterParagraphs
            : fallback.letterParagraphs;
        const letterParagraphs = injectVoiceRecapIntoLetter(letterParagraphsRaw, fallback.letterParagraphs, context.voiceRecap);
        return {
            review: {
...
export function generateMonthlyReview(context) {
    const { userName, tasksCompleted, goalsCompleted, activeGoals, completedGoals, staleGoals, momentsThisMonth, } = context;
    const wins = [];
    if (tasksCompleted > 0) {
    ...
    const domainSummary = topDomains.length > 0
        ? `Focus areas this month: ${topDomains.join(", ")}.`
        : "Set one goal to anchor the month ahead.";
    const parts = [];
    if (userName)
        parts.push(`${userName}, here's your month in review.`);
    else
        parts.push("Here's your month in review.");
...
export function generateQuarterlyReview(context) {
    const { userName, tasksCompleted, goalsCompleted, activeGoals, completedGoals, neglectedDomains, topHabitStreaks, momentsThisQuarter, } = context;
    const wins = [];
    if (tasksCompleted > 0) {
    ...
    const parts = [];
    if (userName)
        parts.push(`${userName}, here's your quarter in review.`);
    else
        parts.push("Here's your quarter in review.");
    parts.push(`${tasksCompleted} tasks completed`);
    if (goalsCompleted > 0) {
        parts.push(`${goalsCompleted} goal${goalsCompleted === 1 ? "" : "s"} finished`);
    }
...
function notice(text, tone, emoji) {
    return { text, tone, emoji };
}
export function generateLifeNotices(ctx) {
    const notices = [];
    const now = Date.now();
    const beliefIds = new Set(ctx.beliefs?.map((b) => b.id) ?? []);
    if (ctx.activeContext?.id === "interview") {
        notices.push(notice("Interview mode — career prep and rest are your top priorities today.", "urgent", "🎯"));
    }
    ...
    if (fitnessHabit && fitnessHabit.streak >= 5) {
        ...
        notices.push(notice(`You're only ${remaining} workouts away from your yearly goal.`, "positive", "🟠"));
    }
    for (const h of ctx.habits) {
        ...
        if (nameMatch && days >= 7) {
            notices.push(notice(`You haven't talked to ${nameMatch[1]} in ${days} days.`, "relationship", "🟣"));
        }
    ...
    for (const m of ctx.moneyItems) {
        if (m.dueDay) {
            ...
            if (daysUntil <= 3 && daysUntil >= 0) {
                notices.push(notice(`${m.title} is due in ${daysUntil} day${daysUntil === 1 ? "" : "s"} — pay or plan today.`, "urgent", "💳"));
            }
            else if (daysUntil <= 21 && daysUntil >= 0) {
                notices.push(notice(`Your ${m.title.toLowerCase()} is due in ${daysUntil} day${daysUntil === 1 ? "" : "s"}.`, "urgent", "🟡"));
            }
        }
    }
    const isFriday = new Date().getDay() === 5;
    if (isFriday && ctx.moneyItems.some((m) => /food|restaurant|dining|uber|entertainment/i.test(m.title))) {
        notices.push(notice(ctx.preferences?.reminderStyle === "direct"
            ? "Friday — you tend to overspend. One budget check before tonight."
            : "Heads up: Fridays are when spending usually spikes for you.", "warning", "📊"));
    }
    ...
    const resumeAge = ctx.applications.length > 0
        ? Math.floor((now - Math.max(...ctx.applications.map((a) => a.updatedAt.getTime()))) / 86400000)
        : 23;
    if (ctx.applications.length > 0 && applied.length === 0) {
        notices.push(notice(`Your resume hasn't changed in ${Math.max(resumeAge, 14)} days.`, "info", "🔵"));
        notices.push(notice("You haven't applied for any jobs recently — one application today moves the needle.", "warning", "🟡"));
    }
    else if (resumeAge >= 14) {
        notices.push(notice(`Your resume hasn't changed in ${resumeAge} days.`, "info", "🔵"));
    }
    if (ctx.staleGoalCount >= 2) {
        notices.push(notice(`${ctx.staleGoalCount} goals haven't moved in a while — let's pick one to revive.`, "warning", "🟡"));
    }
    if (ctx.completedToday === 0 && ctx.habits.some((h) => h.streak > 0)) {
        notices.push(notice("You're sleeping less on routines lately — a small win today rebuilds momentum.", "warning", "🟡"));
    }
    if (notices.length === 0) {
        notices.push(notice("You're building steady momentum — keep today's mission simple and finish strong.", "positive", "🟢"));
    }
    return notices.slice(0, 8);
}
export function generateHeroBriefing(ctx) {
    const firstName = ctx.userName?.split(" ")[0] ?? "there";
    const prefs = ctx.preferences;
    const beliefIds = new Set(ctx.beliefs?.map((b) => b.id) ?? []);
    const effectiveHour = prefs?.peakHours === "night" && ctx.hour < 12
        ? 20
        : prefs?.peakHours === "evening" && ctx.hour < 12
            ? 18
            : ctx.hour;
    const timeGreeting = effectiveHour < 12
        ? `Good morning, ${firstName} 👋`
        : effectiveHour < 17
            ? `Good afternoon, ${firstName} 👋`
            : `Good evening, ${firstName} 👋`;
    const topTask = ctx.pendingMission[0];
    const estMinutes = prefs?.taskLength === "short" ? 15 : prefs?.taskLength === "long" ? 45 : topTask ? 17 : 25;
    ...
    const beliefHint = ctx.beliefs && ctx.beliefs.length > 0
        ? ` I know ${ctx.beliefs
            .slice(0, 2)
            .map((b) => b.label.toLowerCase())
            .join(" and ")} matter to you.`
        : "";
    const chiefOfStaffLine = prefs?.reminderStyle === "direct"
    ...
    const dayAssessment = ctx.completedToday === 0
        ? ctx.hour >= 17
    ...
    const potentialScoreGain = Math.min(9, ctx.pendingMission.length * 2 + (ctx.pendingMission.length > 0 ? 2 : 0));
...
export function generateScoreChangeReasons(scores, completedToday) {
    const labels = {
        career: "Career",
        money: "Money",
        health: "Health",
...
// ─── Interview Prep ─────────────────────────────────────────────────────────
export function defaultInterviewPrep(company, role) {
    return [
...
export function parsePrepChecklist(json, company, role) {
    if (!json)
...
export function interviewPrepProgress(items) {
    if (items.length === 0)
        return 0;
    return Math.round((items.filter((i) => i.done).length / items.length) * 100);
}
export function generateInterviewSuggestions(company, role, items, interviewAt) {
    const tips = [];
...
export { parseVoiceCaptureRules, parseVoiceCaptureWithAI, parseVoiceCaptureBySource, parseNightReflectionRules, parseMorningReflectionRules, parseBrainDumpRules, parseAmbientCaptureRules, searchVoiceCaptures, } from "./voice-capture";
