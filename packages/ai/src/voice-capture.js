...
function detectMood(lower) {
    if (/burned out|exhausted|overwhelmed|stressed|anxious|nervous/i.test(lower))
        return "stressed";
    if (/excited|great|nailed|proud|grateful|happy/i.test(lower))
        return "energized";
    if (/tired|slept \d|only \d hours/i.test(lower))
        return "tired";
    return null;
}
function detectSignals(lower) {
    ...
    if (/burned out|burnout|quit(?:ting)? my job/i.test(lower))
        signals.push("burnout");
    if (/sle(?:pt|ep) \d|only \d hours|insomnia/i.test(lower))
        signals.push("sleep_debt");
    if (/spent \$|costco|shopping|broke|debt/i.test(lower))
        signals.push("spending");
    if (/gained|lost|pounds|weigh/i.test(lower))
        signals.push("body_composition");
    if (/interview|promotion|job|manager|raise/i.test(lower))
        signals.push("career_signal");
    if (/dinner|parents|spouse|friend|family|partner/i.test(lower))
        signals.push("relationship_signal");
    return [...new Set(signals)];
}
function detectDomain(lower) {
    if (/job|career|interview|promotion|boss|manager|work/i.test(lower))
        return "CAREER";
    if (/save|spent|\$|money|budget|debt/i.test(lower))
        return "MONEY";
    if (/sleep|workout|gym|health|weight|pounds/i.test(lower))
        return "HEALTH";
    if (/dinner|parents|spouse|friend|family|partner/i.test(lower))
        return "RELATIONSHIPS";
    if (/learn|study|school|course/i.test(lower))
        return "LEARNING";
    if (/move to|travel|trip/i.test(lower))
        return "TRAVEL";
    return null;
}
...
export function parseVoiceCaptureRules(transcript) {
    ...
    if (sleepMatch) {
        ...
        if (!plan.signals.includes("sleep_debt"))
            plan.signals.push("sleep_debt");
    }
    ...
    const saveMatch = lower.match(/save\s+\$?\s*([\d,]+(?:\.\d+)?)\s*(?:dollars?|bucks)?(?:\s+by|\s+before|\s+until)\s+(.+)/i);
    if (saveMatch) {
        plan.goals.push({
...
export function parseNightReflectionRules(transcript) {
    const base = parseVoiceCaptureRules(transcript);
    const sentences = transcript
        .split(/[.!?]+/)
        .map((s) => s.trim())
        .filter((s) => s.length > 8);
    const wins = sentences.filter((s) => /good|great|proud|nailed|win|accomplish|progress|finally|happy|went well/i.test(s));
    const problems = sentences.filter((s) => /stress|hard|difficult|bad|annoy|frustrat|tired|burn|worried|argument|tough/i.test(s));
    const ideas = sentences.filter((s) => /idea|thinking about|maybe i should|what if/i.test(s));
    const tomorrowPriorities = sentences.filter((s) => /tomorrow|next week|monday|need to|first thing/i.test(s));
    const habits = sentences.filter((s) => /habit|workout|meditat|journal|sleep|routine|steps/i.test(s));
...
export function parseMorningReflectionRules(transcript, ctx) {
    const base = parseVoiceCaptureRules(transcript);
...
export function parseBrainDumpRules(transcript) {
    const base = parseVoiceCaptureRules(transcript);
    ...
    for (const chunk of chunks) {
        ...
        for (const signal of part.signals) {
            if (!base.signals.includes(signal))
                base.signals.push(signal);
        }
    }
    const threadCount = base.memories.length + base.goals.length + base.tasks.length + base.relationshipNotes.length;
    base.summary =
        threadCount > 1
...
export function parseAmbientCaptureRules(transcript, segments) {
    const parts = (segments ?? []).map((s) => s.trim()).filter((s) => s.length > 8);
    if (parts.length === 0)
        return parseBrainDumpRules(transcript);
    const base = {
        summary: "",
        mood: null,
        signals: [],
...
function reflectionSystemPrompt(source, context) {
...
export async function parseVoiceCaptureBySource(transcript, source, context, apiKey, segments) {
    const ruleFallback = source === "night_reflection"
    ...
    const schema = source === "capture" || source === "brain_dump" || source === "ambient_capture"
        ? OUTPUT_SCHEMA
        : REFLECTION_SCHEMA;
    try {
...
export async function parseVoiceCaptureWithAI(transcript, context, apiKey, source = "capture") {
    const { plan } = await parseVoiceCaptureBySource(transcript, source, context, apiKey);
    return plan;
}
export function searchVoiceCaptures(captures, query) {
