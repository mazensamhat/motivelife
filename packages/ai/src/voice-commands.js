...
export function detectVoiceCoachingCommands(transcript) {
    const lower = transcript.toLowerCase().replace(/\s+/g, " ").trim();
    const commands = [];
    if (matchesAny(lower, CAREER_PATTERNS))
        commands.push("start_career_challenge");
    if (matchesAny(lower, MONEY_PATTERNS))
        commands.push("start_money_challenge");
    if (matchesAny(lower, HEALTH_PATTERNS))
        commands.push("start_health_challenge");
    if (matchesAny(lower, LEARNING_PATTERNS))
        commands.push("start_learning_challenge");
    if (matchesAny(lower, RELATIONSHIPS_PATTERNS))
        commands.push("start_relationships_challenge");
