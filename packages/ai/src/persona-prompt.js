export function getGenerationCohort(birthYear) {
    if (!birthYear)
        return null;
...
const COHORT_GUIDANCE = {
    gen_z: "User is Gen Z: mobile-first, short sentences, optional light humor if humor preference is on. Anticipatory not preachy. Respect privacy — never guilt-trip.",
    millennial: "User is Millennial: direct but warm, life-stage aware (career + money + family). One clear next action. Data-backed when statistics mode is on.",
    gen_x: "User is Gen X: pragmatic, no fluff, credible tone. Acknowledge time pressure and retirement/financial stakes when relevant.",
    boomer: "User is 50+: respectful, patient, clear language. Focus on health, relationships, legacy. Never patronizing.",
};
function toneRules(prefs) {
    const lines = [
    ...
    if (peak === "night")
        lines.push("User peaks at night — don't push early-morning framing.");
    if (peak === "morning")
        lines.push("User peaks in the morning — morning framing fits.");
    return lines.join("\n");
}
export function buildPersonaSystemPrompt(layers) {
    const cohort = getGenerationCohort(layers.birthYear);
    const beliefLabels = layers.beliefs.map((b) => b.label).join(", ") || "not yet captured";
    const graphSummary = layers.graph?.destination?.label != null
    ...
    const learned = layers.learnedToday.length > 0
        ? `Patterns learned recently: ${layers.learnedToday.slice(0, 4).join("; ")}.`
        : "";
    const streakLine = layers.lifeEngineStreak && layers.lifeEngineStreak > 0
...
export function buildPersonaUserPayload(taskContext, outputSchema) {
...
export function computeLifeEngineStreakUpdate(lastCompletedAt, currentStreak, bestStreak, now = new Date()) {
...
export function getLifeEngineStreakStatus(lastCompletedAt, currentStreak, freezesRemaining, now = new Date()) {
    const todayStart = new Date(now);
    todayStart.setHours(0, 0, 0, 0);
    if (!lastCompletedAt) {
