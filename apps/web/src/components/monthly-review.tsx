...
export function MonthlyReviewPanel() {
  ...
  async function load(refresh = false) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/monthly-review${refresh ? "?refresh=true" : ""}`);
      ...
      setError("Could not load monthly review.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Card className="border-brand-purple/30 bg-gradient-to-br from-forward-50 to-white">
