...
# Shared fallback (must match the client that issued each brand's refresh token):
MARKETING_YOUTUBE_CLIENT_ID=
MARKETING_YOUTUBE_CLIENT_SECRET=
# Per-brand OAuth client (preferred when brands use different Google OAuth clients):
# MARKETING_MOTIVELIFE_YOUTUBE_CLIENT_ID=
# MARKETING_MOTIVELIFE_YOUTUBE_CLIENT_SECRET=
# MARKETING_MOTIVEFX_YOUTUBE_CLIENT_ID=
# MARKETING_MOTIVEFX_YOUTUBE_CLIENT_SECRET=
...
If publish fails with `unauthorized_client`, the refresh token and `*_CLIENT_ID`/`SECRET` do not match — set per-brand `MARKETING_<BRAND>_YOUTUBE_CLIENT_*` to the same OAuth client used in `youtube-oauth.mjs`.
