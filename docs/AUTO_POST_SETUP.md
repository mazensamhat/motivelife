...
Ops uploads 9:16 Shorts via **YouTube Data API v3**. Quota ≈ **1,600 units/upload**.

**Important:** the refresh token only works with the **same** OAuth client ID/secret that issued it. If Production `GOOGLE_CLIENT_*` is a different app than the one used in `youtube-oauth.mjs`, set per-brand clients (avoids `unauthorized_client`):

`MARKETING_MOTIVELIFE_YOUTUBE_CLIENT_ID` / `MARKETING_MOTIVELIFE_YOUTUBE_CLIENT_SECRET`

1. [Google Cloud Console](https://console.cloud.google.com/) → enable **YouTube Data API v3**
2. OAuth **Web** client — authorized redirect URI: `http://127.0.0.1:8765/callback` (and `http://localhost:8765/callback`)
3. Run one-time OAuth as the Google account that **owns @MotiveLife-ai** (not the MotiveFX channel):

```powershell
cd packages/marketing-agent
$env:MARKETING_YOUTUBE_CLIENT_ID="..."   # MotiveLife Web client
...
```
MARKETING_MOTIVELIFE_YOUTUBE_CLIENT_ID=...     # same client as oauth script
MARKETING_MOTIVELIFE_YOUTUBE_CLIENT_SECRET=...
MARKETING_MOTIVELIFE_YOUTUBE_CHANNEL_ID=UCzjdFghiI1akeuVeSERu21A
MARKETING_MOTIVELIFE_YOUTUBE_REFRESH_TOKEN=1//...
```

(Alternatively set shared `MARKETING_YOUTUBE_CLIENT_ID`/`SECRET` if MotiveFX uses that same client.)
...
Save the refresh token in a password manager. Use a **separate** OAuth run for MotiveFX (`node ./scripts/youtube-oauth.mjs motivefx`) so each brand has its own refresh token — and matching `MARKETING_MOTIVEFX_YOUTUBE_CLIENT_*` if that token came from a different OAuth client.
...
| `MARKETING_YOUTUBE_CLIENT_ID` / `SECRET` | Shared YouTube OAuth fallback (or `GOOGLE_CLIENT_*`) |
| `MARKETING_MOTIVELIFE_YOUTUBE_CLIENT_ID` / `SECRET` | Preferred — must match client that issued MotiveLife refresh token |
